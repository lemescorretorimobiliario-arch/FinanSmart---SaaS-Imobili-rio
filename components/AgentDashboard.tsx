...
const AgentDashboard: React.FC<Props> = ({ user, onSelectLead, onUpgrade }) => {
  ...
  return (
    <div className="p-3 sm:p-4 md:p-6 max-w-[1400px] mx-auto space-y-3 sm:space-y-4 md:space-y-6 animate-fade-in pb-20 md:pb-8 font-sans">

      {/* HEADER SECTION */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div>
          <h1 className="text-lg sm:text-xl md:text-2xl font-black text-slate-900 tracking-tighter">Gestão de Carteira</h1>
          <p className="text-slate-500 font-medium text-[10px] sm:text-xs">Transforme leads em contratos assinados.</p>
        </div>

        <div className="flex items-center gap-2">
          <div className="relative group flex-1 md:flex-initial">
            <Search className="absolute left-2.5 sm:left-3 top-1/2 -translate-y-1/2 w-3.5 sm:w-4 h-3.5 sm:h-4 text-slate-300" />
            <input
              type="text"
              placeholder="Buscar lead..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="pl-8 sm:pl-10 pr-3 sm:pr-4 py-2 bg-white border border-slate-200 rounded-lg sm:rounded-xl text-[11px] sm:text-xs font-bold focus:ring-2 focus:ring-blue-100 outline-none w-full md:w-48 lg:w-64"
            />
          </div>
          <button
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
        <div className="bg-white p-3 sm:p-4 rounded-lg sm:rounded-xl border border-slate-100 shadow-sm">
          <p className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Total de Leads</p>
          <p className="text-lg sm:text-xl font-black text-slate-900">{leads.length}</p>
        </div>
      ...
      {/* KANBAN BOARD */}
      <div className="flex gap-3 sm:gap-4 overflow-x-auto pb-4 items-start min-h-[400px] sm:min-h-[500px]">
        {PIPELINE_STEPS.map(step => {
          const stepLeads = filteredLeads.filter(l => l.status === step.id);
          return (
            <div key={step.id} className={`min-w-[260px] sm:min-w-[280px] md:min-w-[300px] w-[260px] sm:w-[280px] md:w-[300px] flex-shrink-0 flex flex-col gap-2 sm:gap-3 p-2.5 sm:p-3 rounded-xl sm:rounded-2xl ${step.bg}`}>
              <div className={`p-2.5 sm:p-3 rounded-lg sm:rounded-xl flex items-center justify-between border shadow-sm ${step.headerBg} ${step.text} ${step.border}`}>
                <div className="flex items-center gap-1.5 sm:gap-2">
                  <step.icon className="w-3.5 sm:w-4 h-3.5 sm:h-4" />
                  <span className="font-black text-[9px] sm:text-[10px] uppercase tracking-widest">{step.label}</span>
                </div>
                <span className="bg-white/60 px-1.5 sm:px-2 py-0.5 rounded-lg text-[9px] sm:text-[10px] font-black">{stepLeads.length}</span>
              </div>

              <div className="space-y-2 sm:space-y-3">
                {stepLeads.map(lead => (
                  <div key={lead.id} className="bg-white p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-100 shadow-sm hover:shadow-xl hover:border-blue-200 transition-all group cursor-pointer" onClick={() => onSelectLead?.(lead)}>
                    <div className="flex justify-between items-start mb-2 sm:mb-3">
                      <h4 className="font-black text-slate-900 text-xs sm:text-sm pr-2">{lead.name}</h4>
                    ...
                    <div className="mb-3 sm:mb-4">
                      <p className="text-[11px] sm:text-xs font-black text-emerald-600">{lead.interest}</p>
                    </div>
                    <div className="flex items-center justify-between pt-2.5 sm:pt-3 border-t border-slate-50">
                      <span className="text-[9px] sm:text-[10px] font-bold text-slate-400 tracking-tighter">{new Date(lead.date).toLocaleDateString()}</span>
