...
const AgentDashboard: React.FC<Props> = ({ user, onSelectLead, onUpgrade }) => {
  ...
  return (
    <div className="p-4 md:p-8 max-w-[1400px] mx-auto space-y-8 animate-fade-in pb-24 font-sans">

      {/* HEADER SECTION - Compact */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tighter">CRM de Vendas</h1>
            <span className="px-2 py-0.5 bg-blue-50 text-blue-600 rounded-full text-[10px] font-bold uppercase tracking-wide">Beta</span>
          </div>
          <p className="text-slate-500 font-medium text-sm">Gerencie seus leads e acompanhe o funil.</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative group flex-1 md:flex-none">
            ...
            <input
              ...
              className="pl-10 pr-4 py-2 bg-white border border-slate-200 rounded-xl text-xs font-bold focus:ring-2 focus:ring-blue-100 outline-none w-full md:w-64 shadow-sm transition-all"
            />
          </div>
          ...
          <button
            ...
            className="bg-slate-900 text-white px-5 py-2.5 rounded-xl font-bold uppercase tracking-wider text-xs hover:bg-blue-600 transition-all shadow-lg active:scale-95 flex items-center gap-2"
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Total Leads</p>
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Novos</p>
            <p className="text-xl font-black text-slate-900">{newLeadsCount}</p>
          </div>
        </div>
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4 relative overflow-hidden group">
          ...
          <div className="relative z-10">
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">VGV Potencial</p>
            <p className="text-xl font-black text-slate-900">{formatBRL(potentialValue)}</p>
          </div>
          <div className="absolute top-0 right-0 w-24 h-24 bg-emerald-50 rounded-full blur-2xl -translate-y-1/2 translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity"></div>
        </div>

        {/* PLAN STATUS */}
        <div className={`p-4 rounded-2xl border flex items-center justify-between gap-4 ${user.plan === 'PRO' ? 'bg-slate-900 text-white border-slate-800' : 'bg-white border-slate-100'}`}>
          <div className="flex items-center gap-3">
            <div className={`p-2 rounded-lg ${user.plan === 'PRO' ? 'bg-white/10 text-yellow-400' : 'bg-blue-50 text-blue-600'}`}>
              {user.plan === 'PRO' ? <Star className="w-4 h-4 fill-yellow-400" /> : <Zap className="w-4 h-4" />}
            </div>
            <div>
              <p className={`text-[9px] font-black uppercase tracking-widest mb-0.5 ${user.plan === 'PRO' ? 'text-slate-400' : 'text-slate-400'}`}>Seu Plano</p>
              <p className="text-sm font-black">{user.plan === 'PRO' ? 'PRO Ativo' : 'Versão Free'}</p>
            </div>
          </div>
          {user.plan === 'FREE' && (
            <button onClick={onUpgrade} className="px-3 py-1.5 bg-blue-600 text-white rounded-lg text-[9px] font-bold uppercase tracking-widest hover:bg-blue-700 transition-colors">
              Upgrade
            </button>
          )}
        </div>
