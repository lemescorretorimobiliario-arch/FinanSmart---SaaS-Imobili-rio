...
const AgentDashboard: React.FC<Props> = ({ user, onSelectLead }) => {
  ...
  const confirmDeleteLead = async () => {
      ...
      try {
          ...
          alert(`Erro ao excluir lead: ${err.message || "Tente novamente."}`);
      } finally {
          setIsDeleting(false);
      }
  };
                  ...
                  >
                    ...
                    <td className="px-6 py-4" onClick={(e) => e.stopPropagation()}>
                      <select 
                        ...
                        className={`text-xs font-bold py-1.5 pl-2 pr-8 rounded-lg appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-opacity-50 transition-all ${
                            lead.status === 'NOVO' ? 'bg-blue-100 text-blue-700 focus:ring-blue-500' :
