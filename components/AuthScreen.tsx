...
const AuthScreen: React.FC<AuthScreenProps> = ({ onLogin }) => {
  ...
  const [userType, setUserType] = useState<UserRole>(UserRole.CLIENTE);
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md z-10 overflow-hidden flex flex-col animate-fade-in-up">
        ...
        <div className="px-8 mb-6">
          <div className="bg-slate-100 p-1 rounded-xl flex relative">
            ...
            <div
              className={`absolute top-1 bottom-1 w-[calc(50%-4px)] bg-white rounded-lg shadow-sm transition-all duration-300 ease-out ${mode === 'LOGIN' ? 'left-1' : 'left-[calc(50%+4px)]'
                }`}
            ></div>

            <button
              onClick={() => { setMode('LOGIN'); setError(''); }}
              className={`flex-1 py-2 text-sm font-bold rounded-lg relative z-10 transition-colors ${mode === 'LOGIN' ? 'text-slate-800' : 'text-slate-400 hover:text-slate-600'
                }`}
            >
              Entrar
            </button>
            <button
              ...
              className={`flex-1 py-2 text-sm font-bold rounded-lg relative z-10 transition-colors ${mode === 'REGISTER' ? 'text-slate-800' : 'text-slate-400 hover:text-slate-600'
                }`}
            >
              Criar Conta
            </button>
          ...
          {mode === 'REGISTER' && (
            <div className="space-y-2 animate-fade-in pt-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  type="button"
                  onClick={() => setUserType(UserRole.CLIENTE)}
                  className={`p-3 rounded-xl border-2 flex flex-col items-center gap-2 transition-all ${userType === UserRole.CLIENTE ? 'border-blue-600 bg-blue-50 text-blue-700' : 'border-slate-100 text-slate-400 hover:border-slate-200'
                    }`}
                >
                ...
                <button
                  type="button"
                  onClick={() => setUserType(UserRole.CORRETOR)}
                  className={`p-3 rounded-xl border-2 flex flex-col items-center gap-2 transition-all ${userType === UserRole.CORRETOR ? 'border-blue-600 bg-blue-50 text-blue-700' : 'border-slate-100 text-slate-400 hover:border-slate-200'
                    }`}
                >
