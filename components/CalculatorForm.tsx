...
const CalculatorForm: React.FC<Props> = ({ data, onChange, onSimulate }) => {
  ...
  return (
    <div className="bg-white md:h-full flex flex-col h-full relative">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 md:px-6 md:py-6 custom-scrollbar pb-24 md:pb-6">
                ...
                <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full pl-2 pr-6 py-2 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none font-bold text-slate-800 text-base"
                    />
                ...
                {data.extraAmortizationMonthly && data.extraAmortizationMonthly > 0 ? (
                  <div className="flex gap-2">
                    <button
                      ...
                      className={`flex-1 py-2 text-xs font-bold rounded-lg border transition-all flex flex-col items-center gap-1 ${(data.extraAmortizationStrategy || 'REDUCE_TERM') === 'REDUCE_TERM'
                    ...
                    <button
                      ...
                      className={`flex-1 py-2 text-xs font-bold rounded-lg border transition-all flex flex-col items-center gap-1 ${data.extraAmortizationStrategy === 'REDUCE_INSTALLMENT'
                        ? 'bg-amber-500 text-white border-amber-500'
                        : 'bg-white text-slate-500 border-slate-200'
                        }`}
                    >
      ...
      {/* Footer Actions */}
      <div className="absolute bottom-0 left-0 right-0 p-4 md:p-5 border-t border-slate-100 flex justify-between items-center bg-white/90 backdrop-blur-sm flex-shrink-0 z-30">
        <button
          onClick={prevStep}
          disabled={currentStep === 1}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-colors text-sm ${currentStep === 1 ? 'text-slate-300 cursor-not-allowed' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-50'
            }`}
        >
