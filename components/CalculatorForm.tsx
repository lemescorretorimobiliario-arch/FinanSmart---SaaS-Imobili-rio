...
const CalculatorForm: React.FC<CalculatorFormProps> = ({ data, onChange, onSimulate, user }) => {
  ...
  return (
    <div className="h-full flex flex-col bg-white">
      ...
      <div className="px-5 py-2 border-b border-slate-100 flex-shrink-0 bg-white/80 backdrop-blur-sm z-20">
        <div className="flex justify-between items-center mb-1.5">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
            {steps[currentStep - 1].label}
          </span>
          <span className="text-[9px] font-bold text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded border border-blue-100/50 uppercase tracking-wider">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6 custom-scrollbar scroll-smooth">
        <div className="max-w-[340px] mx-auto">
          ...
          {currentStep === 1 && (
            <div className="space-y-5 animate-fade-in-up">
              <div className="text-center mb-6">
                <h2 className="text-sm font-bold text-slate-900">Planejamento Patrimonial</h2>
                <p className="text-[10px] text-slate-500 mt-1">Defina os valores base para o seu financiamento</p>
              </div>

              <div className="finan-card-premium p-4">
                <div className="space-y-4">
                  <div>
                    <label className="text-[9px] font-bold uppercase text-slate-500 tracking-wider block mb-1.5">Valor do Imóvel</label>
                    <div className="relative">
                      <input
                        type="text"
                        value={formatCurrencyValue(data.propertyValue)}
                        onChange={(e) => handleCurrencyChange('propertyValue', e.target.value)}
                        className="w-full text-lg font-bold text-slate-900 bg-slate-50 border border-slate-100 rounded-lg px-3 py-2 focus:ring-1 focus:ring-blue-500/30 outline-none transition-all"
                      />
                    </div>
                  </div>
                  ...
                  <div className="pt-2 border-t border-slate-50">
                    <div className="flex justify-between items-center mb-1.5">
                      <label className="text-[9px] font-bold uppercase text-slate-500 tracking-wider">Entrada</label>
                      <span className="text-[9px] font-bold text-blue-600 px-1 rounded bg-blue-50">{Math.round((data.downPayment / data.propertyValue) * 100)}%</span>
                    </div>
                    <input
                      type="text"
                      value={formatCurrencyValue(data.downPayment)}
                      onChange={(e) => handleCurrencyChange('downPayment', e.target.value)}
                      className="w-full text-base font-bold text-slate-900 bg-slate-50 border border-slate-100 rounded-lg px-3 py-2 focus:ring-1 focus:ring-blue-500/30 outline-none transition-all"
                    />
                  </div>
                </div>
              </div>

              <div className="p-3 bg-blue-50/30 border border-blue-100/30 rounded-xl flex gap-x-2.5 items-start">
          ...
          {currentStep === 2 && (
            <div className="space-y-5 animate-fade-in-up">
              <div className="finan-card p-4 border-slate-200/40">
                <div className="flex justify-between items-center mb-4">
                  <label className="text-[9px] font-bold uppercase text-slate-600 tracking-wider px-2 py-0.5 bg-slate-100 rounded">Prazo</label>
                  <span className="text-sm font-bold text-indigo-600">{data.termYears} anos</span>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="finan-card p-3">
                  <label className="text-[9px] font-bold uppercase text-slate-500 mb-1.5 block">Taxa Juros</label>
                ...
                <div className="finan-card p-3">
                  <label className="text-[9px] font-bold uppercase text-slate-500 mb-1.5 block">Amortização</label>
          ...
          {currentStep === 3 && (
            <div className="space-y-5 animate-fade-in-up">
              <div className="text-center pt-2">
                <h2 className="text-sm font-bold text-slate-900">Estratégia de Quitação</h2>
              ...
              <div className="finan-card-premium p-5 border-blue-100/50">
                <label className="text-[9px] font-bold uppercase text-blue-600 tracking-wider mb-2 block text-center">Aporte Mensal Adicional</label>
                <input
                  type="text"
                  ...
                  className="w-full bg-slate-50 border border-slate-100 rounded-xl text-xl font-bold text-slate-900 text-center py-3 focus:outline-none focus:ring-1 focus:ring-blue-500/20"
                  placeholder="R$ 0,00"
                />
              </div>
      ...
      <div className="px-6 py-4 border-t border-slate-50 bg-white">
        <div className="flex gap-3 max-w-[340px] mx-auto items-center">
          {currentStep > 1 && (
            <button
              onClick={prevStep}
              className="w-10 h-10 rounded-xl bg-slate-50 text-slate-400 border border-slate-100 flex items-center justify-center hover:bg-slate-100 transition-all active:scale-90"
            >
          ...
          {currentStep < steps.length ? (
            <button
              onClick={nextStep}
              className="flex-1 bg-slate-900 text-white h-11 rounded-xl font-bold text-xs uppercase tracking-widest shadow-lg shadow-slate-900/10 flex items-center justify-center gap-2 active:scale-[0.98] transition-all"
            >
              Próximo
            ...
            <button
              onClick={onSimulate}
              className={`flex-1 ${user ? 'premium-gradient' : 'bg-slate-900'} text-white h-11 rounded-xl font-bold text-xs uppercase tracking-widest shadow-lg flex items-center justify-center gap-2 active:scale-[0.98] transition-all`}
            >
