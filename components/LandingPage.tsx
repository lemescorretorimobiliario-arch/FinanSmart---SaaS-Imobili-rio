    ...
    return (
        <div className="bg-slate-50 font-sans text-slate-900 overflow-x-hidden selection:bg-blue-100 selection:text-blue-900">
            ...
            <header className="fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-xl border-b border-slate-200/60">
                <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 h-14 sm:h-16 md:h-14 flex items-center justify-between">
                    <div className="flex items-center gap-1.5 sm:gap-2 group cursor-pointer" onClick={() => navigate('/')}>
                        <div className="premium-gradient p-0.5 sm:p-1 rounded-lg shadow-blue-500/10 group-hover:scale-105 transition-all duration-300">
                            <Layout className="w-3 sm:w-3.5 h-3 sm:h-3.5 text-white" />
                        </div>
                        <span className="font-bold text-xs sm:text-sm tracking-tight text-slate-900">Finan<span className="text-blue-600">Smart</span></span>
                    </div>
                    ...
                    <div className="flex items-center gap-2 sm:gap-3 md:gap-4">
                        <button
                            ...
                            className="text-[9px] sm:text-[10px] font-bold text-slate-500 hover:text-slate-900 transition-colors uppercase tracking-wider"
                        >
                            Entrar
                        </button>
                        <button
                            onClick={() => navigate('/simulador')}
                            className="btn-primary px-3 sm:px-4 py-1.5 text-[9px] sm:text-[10px]"
                        >
                            Ver Simulador
                        </button>
                    </div>
            ...
            {/* Hero Section */}
            <section className="relative pt-20 sm:pt-24 md:pt-36 pb-12 sm:pb-16 md:pb-24 overflow-hidden bg-white">
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
                    <div className="inline-flex items-center gap-1.5 sm:gap-2 px-2 sm:px-2.5 py-0.5 rounded-full bg-blue-50 border border-blue-100 text-blue-600 font-bold text-[8px] sm:text-[9px] uppercase tracking-widest mb-4 sm:mb-6 animate-fade-in-up">
                        <Star className="w-2 sm:w-2.5 h-2 sm:h-2.5 fill-blue-600" />
                        O Simulador nº 1 do Mercado Imobiliário
                    </div>

                    <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-black text-slate-950 tracking-tight mb-3 sm:mb-4 leading-[1.1] max-w-4xl mx-auto animate-fade-in-up">
                        Feche mais vendas com <br />
                        <span className="text-blue-600">Inteligência Financeira.</span>
                    </h1>

                    <p className="text-xs sm:text-sm md:text-base text-slate-500 max-w-2xl mx-auto mb-6 sm:mb-8 leading-relaxed font-medium animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-center gap-2.5 sm:gap-3 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
                        <button
                            onClick={() => navigate('/simulador')}
                            className="w-full sm:w-auto px-5 sm:px-6 py-2.5 sm:py-3 bg-slate-900 text-white rounded-lg sm:rounded-xl font-bold text-xs sm:text-sm hover:bg-blue-600 transition-all hover:-translate-y-1 active:scale-95 flex items-center justify-center gap-2"
                        >
                            Começar Simulação
                        ...
                        <button
                            onClick={() => navigate('/login')}
                            className="w-full sm:w-auto px-5 sm:px-6 py-2.5 sm:py-3 bg-white text-slate-700 border border-slate-200 rounded-lg sm:rounded-xl font-bold text-xs sm:text-sm hover:bg-slate-50 transition-all active:scale-95"
                        >
                            Área do Corretor
                        </button>
                    </div>
            ...
            <section id="features" className="py-12 sm:py-16 md:py-24 bg-slate-50 border-t border-slate-200/60">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="text-center max-w-3xl mx-auto mb-8 sm:mb-10 md:mb-12">
                        <span className="text-[8px] sm:text-[9px] font-bold text-blue-600 uppercase tracking-widest bg-blue-50 px-2 py-0.5 rounded-full border border-blue-100 mb-2 inline-block">Funcionalidades</span>
                        <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-slate-900 mb-2 sm:mb-3 tracking-tight">Tudo que você precisa para vender mais.</h2>
                        <p className="text-[11px] sm:text-xs text-slate-500 font-medium leading-relaxed">Economize horas de cálculos manuais e erros de estimativa.</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4 md:gap-6">
                        {[
                            ...
                            <div key={i} className="finan-card p-4 sm:p-5 md:p-6 group hover:border-blue-200">
                                <div className="w-7 h-7 sm:w-8 sm:h-8 bg-slate-50 text-slate-600 rounded-lg flex items-center justify-center mb-3 sm:mb-4 group-hover:bg-blue-600 group-hover:text-white transition-all">
                                    <f.icon className="w-3.5 sm:w-4 h-3.5 sm:h-4" />
                                </div>
                                <h3 className="text-xs sm:text-sm font-bold text-slate-900 mb-1 sm:mb-1.5">{f.title}</h3>
                                <p className="text-[10px] sm:text-[11px] text-slate-500 leading-relaxed font-medium">{f.desc}</p>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <section id="pricing" className="py-12 sm:py-16 md:py-24 bg-white">
                <div className="max-w-7xl mx-auto px-4">
                    <div className="text-center mb-8 sm:mb-10 md:mb-12">
                        <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-slate-900 tracking-tight">Transparência Total.</h2>
                        <p className="text-[11px] sm:text-xs text-slate-500 mt-1 font-medium leading-relaxed">Use grátis ou acelere com o plano PRO.</p>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 md:gap-6 max-w-4xl mx-auto">
                        {/* Free */}
                        <div className="finan-card p-5 sm:p-6 flex flex-col">
